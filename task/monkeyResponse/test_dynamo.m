...
n_window = 1;
wd_osc(1) = Screen('OpenWindow', n_window, [], [6+wh_px 1 6+2*wh_px 1+ww_px]);
wd_osc(2) = Screen('OpenWindow', n_window, [], [1 1 1+wh_px 1+ww_px]);
...
while exit==0    
    for iChannel = 1:2
        ...
        [Values, Times] = ReadGripValue(DeviceName, Handle, Channels(iChannel));
        sampled(iChannel,:) = [sampled(iChannel,[2:end]), Values];
        ...
        if keyisdown==1 && keycode(KbName('c'))
           [Fs, VDiv, Offset, Channels(iChannel)] = getUserParam(Fs, VDiv, Offset, Channels(iChannel));
        end

        % plot into a window
        xy = [1:ww_px; sampled(iChannel,:)];
        PlotDataOnOsc(wd_osc(iChannel), xy, Offset, VDiv, wh_px, ww_px)

        % display parametersc
        DispParam(wd_osc(iChannel), Offset, VDiv, xy(2,end), eFs, mean(mFs))

        % get timing, flush event and impose a sampling rate
        Screen('Flip', wd_osc(iChannel));
