...
while task_exit==0    
    ...
    if  isnan(rewards(:,ntrial))
        ...
        Screen(window,'Flip'); 
        Screen('TextSize', window, 20);
        [width,hight]=RectSize(Screen('TextBounds',window,'Sélectionnez les types d'' offres'));
        Screen('DrawText',window,'Sélectionnez les types d'' offres',x-width/2,y,[100 100 100]);
        
        [width,hight]=RectSize(Screen('TextBounds',window,'1D: efforts & récompenses'));
        Screen('DrawText',window,'1D: efforts & récompenses',x-width/2-x/2,y+300,[100 100 100]);
        
        [width,hight]=RectSize(Screen('TextBounds',window,'2D: efforts & récompenses congruents'));
        Screen('DrawText',window,'2D: efforts & récompenses congruents',x-width/2+x/2,y+300,[100 100 100]);
        
        [width,hight]=RectSize(Screen('TextBounds',window,'2D: efforts & récompenses indépendants'));
        Screen('DrawText',window,'2D: efforts & récompenses indépendants',x-width/2,y+400,[100 100 100]);
        ...
        if  keyCode(key.left)==1
            Block1 = [ 1,1,2,2, 3,3,4,4, 1,1,2,2,3,3,4,4];
            Block2 = [ 4,3,4,3, 2,1,2,1, 1,1,2,2,3,3,4,4];
        ...
        elseif  keyCode(key.right)==1
            Block1 = [ 1,1,2,2, 3,3,4,4, 2,2,3,3, 2,2,2,2];
            Block2 = [ 4,3,4,3, 2,1,2,1, 2,2,2,2, 2,3,3,2];
            
            Block3 = [ 2,2,3,3, 2,2,2,2, 1,1,2,2, 3,3,4,4];
            Block4 = [ 2,2,2,2, 2,3,3,2, 4,3,4,3, 2,1,2,1];
            
            rewards(:,ntrial:ntrial+nTrialByBlock-1) = [ Block1(index) ; Block2(index) ]; 
            efforts(:,ntrial:ntrial+nTrialByBlock-1) = [ Block3(index) ; Block4(index) ];   
        elseif keyCode(key.space)==1 
            Block1 = [ 1,1,2,2, 3,3,4,4, 2,2,3,2, 2,3,2,2];
            Block2 = [ 4,3,4,3, 2,1,2,1, 2,3,2,2, 2,2,3,2];
            
            Block3 = [ 2,2,3,2, 2,3,2,2, 1,1,2,2, 3,3,4,4];
            Block4 = [ 2,3,2,2, 2,2,3,2, 4,3,4,3, 2,1,2,1];
