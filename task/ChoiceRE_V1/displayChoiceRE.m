...
%% define
   % data acquisition parameters
    ...
    fig = figure; set(fig,'Color',[1,1,1]);set(fig,'Name','PARTICIPATION PATTERN (all trials)')
%     set(fig,'Position',[5 5 15 30]);
    
        % histrogram
        subplot(2,2,[1]);hold on;
%             histogram(data.errorType,'FaceColor',[0.5 0.5 0.5],'EdgeColor','k');
%             hist(data.errorType);
            empirDist = zeros(1,5);
...
%                   'TickLength',[ 0 0 ]);
                  %                 'XTickLabelRotation',40,'TickLength',[ 0 0 ]);
