function [data,gripdata,gripbaseline] = ChoiceRE_v120_task(root,calib,reward_unit_duration,effort_unit_duration)
...
% current specifications
    % only extreme effort options offered
        currentEffort = 2; % it could be 2 or 3 or 4
...
%         effortBlock1 = [ repmat(1,1,nTrialByBlock/2),repmat(4,1,nTrialByBlock/2)]; 
%         effortBlock2 =  fliplr(effortBlock1); 
        effortBlock1 = [1,1,1,2,2,2,3,3,3,4,4,4];
        effortBlock2 = [2,3,4,1,3,4,1,2,4,1,2,3];
%         effortBlock2 = [  repmat(4,1,nTrialByBlock/2),repmat(1,1,nTrialByBlock/2)]; 
    % only currentReward offered
        currentReward = 2; % it could be 2 or 3 or 4
...
%         sideBlock = [repmat(1,1,nTrialByBlock/2) , repmat(2,1,nTrialByBlock/2)];
        sideBlock = [repmat([1 , 2],1,nTrialByBlock/2)];
    ...
    while GetSecs<(prepareTime + redSpotDuration(ntrial) + anticipationduration)
        ...
        for s = 1:2
            ...
            if level(s)>=anticipationCriterion % anticipation error
                error(ntrial)= 2 ;
                if choice(ntrial)==0; choice(ntrial)=s; end;
            end
        end
