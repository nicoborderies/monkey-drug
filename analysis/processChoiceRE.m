...
%% define
    ...
    % data selection
        ...
        selectionUnmissed = ( data.errorType~=1 & selectionUnrepeated);
        selectionUnpremature = ( data.errorType~=2 & selectionUnrepeated);
        selectionUnswitched = ( data.errorType~=3 & selectionUnrepeated);
        selectionIncorrect = ( data.errorType~=4 & selectionUnrepeated);

        selectionChosen = ( selectionUnmissed & selectionUnpremature & selectionUnswitched  );
...
%% descriptive
    ...
    % bivariate
        ...
        selection = selectionUnmissed;
            y = (data.errorType(selection)==2);
                x = data.ordinalRewardLeft(selection) + data.ordinalRewardRight(selection);
                result.descriptive.quantile_prematureError_sumReward  = unique(x);
                result.descriptive.mean_prematureError_by_sumReward= tapply( y,{ x },@nanmean, {'discrete'});
                result.descriptive.sem_prematureError_by_sumReward= tapply( y ,{ x },@sem, {'discrete'});
    ...
    % choice observed
        selection = selectionChosen;
              % fit choice
                ...
                % predictions
                    y2 = result.inferential.glm_choice.Fitted.Response;
%                     data.predictedChoice= y;
                    result.inferential.prediction_sideChoice_by_deltaOrdinalReward = tapply( y2,{ x1 },@nanmean, {'discrete'});
                    result.inferential.prediction_sideChoice_by_deltaOrdinalEffort = tapply( y2,{ x2 },@nanmean, {'discrete'});
                ...
                result.summary.BCA =  nanmean([ nanmean(y(y2>0.5)) , nanmean(1-y(y2<0.5)) ]);
