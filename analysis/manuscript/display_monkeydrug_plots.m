...
yvar_name = 'choice = HE (%)'; % participation (%), choice = HR (%), choice = HE (%), force peak (%fmax), response time (s)
...
trt2plot = {'clonidine','placebo','atomoxetine'}; % {'clonidine','placebo','atomoxetine'};
isBin = 0;
dispPrediction = 0;
effect = 'random';
disp_x = 1;
...
[g1,subject,sessionNumber,xbin] = findgroups(subject,sessionNumber,xbin);
...
% 2-level statistics
g2 = findgroups(subject,treatment,xbin);
g3 = findgroups(treatment,xbin);
 for ivar = 1:numel(variableNames)
...
switch effect
    case 'random'
        ...
        for isub = 1:numel(unique(subject))
            ...
            g(1,isub) = gramm('x',xvar,'y',yvar,'color',treatment,'subset',subset);
